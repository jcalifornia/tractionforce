...
implementation, we use a second-order quadratic cone solver.
...
assume $\bsigma\in L^1(\Omega)$ such that $\int \bsigma(\r)\d\r = \mathbf{0}$, then
...
\Big[f_{xy}(\Delta x_j^{-},\Delta y_k^+)-f_{xy}(\Delta x_j^{-},\Delta y_k^-)
- f_{xy}(\Delta x_j^{+},\Delta y_k^+) + f_{xy}(\Delta x_j^+,\Delta y_k^-) \Big]x_n \nonumber\\
\: & \qquad -\Big[f_{xy}^x(\Delta x_j^{+},\Delta y_j^+) - f_{xy}^x(\Delta x_j^{+},\Delta y_j^-)
-f_{xy}^x(\Delta x_j^{-},\Delta y_j^+)+f_{xy}^x(\Delta x_j^{-},\Delta y_j^-)\Big]
\end{align}
%
where
...
last line of Eq.~\ref{eq:rhoeq}, one sees that it in order to show
that the magnitude of $u_x(\r)$ is ${\cal o}(|\r|^{-q})$, it
