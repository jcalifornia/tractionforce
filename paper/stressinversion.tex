...
\begin{equation}
\tilde{\Phi}_{\rm TV} =  
\int_\Omega \vert \nabla(\vert \f\vert)\vert\dd x\dd y.
\label{eq:PhiTVTr}
\end{equation}
...
\caption{\textbf{Unidirectional displacement measurements} Sets of
  reconstructions using displacements under only $\hat{x}$ or
  $\hat{y}$ directions. DESCRIPTION OF WHAT HAPPENS.}
\label{XYONLY}
\end{figure}
...
resonstructions using $\tilde{\Phi}_{L^{1}}$, but without force or
torque constraints.

\begin{figure}
\includegraphics[width=\linewidth]{fig3}
\caption{\textbf{Constraints matter.} Physical constraints are not satisfied unless they are enforced. Plotted are best reconstructions
