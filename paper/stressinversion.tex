...
substrate are realized through {\it e.g.}, lamellipodia, filopodia,
...
the cell to attach to a non-flat interface.  The simplest method
compatible with a flat interface relies on measuring the displacement
...
adhesion structures. To image such sub-cellular stress structures, high
...
mean force of a molecular bond, a constraint on differentiability is
...
To this end one may employ a variant of a penalities used often in
...
forms of its total variation, respectively.  

Since these
regularization terms are not based on any fundamental physical law,
there is some freedom of in choosing their form.  However,
we do not want the choice of parameterization for the data grid
to affect the reconstruction results. Hence,
the regularization terms should not induce any additional
...
sampled.  In this manuscript, we will focus on the isotropic
$L^1$ norm
\begin{equation}
...
we define the the penalized optimization problem
...
In our examples, we used $a_{12} = 10^{-4}$, $a_{23} = 2\times10^{-4}$, and $g_4 = 9\times10^{-5}$.
 We generated displacement fields by solving the forward problem of
(\textit{i.e.}, Eq.~\ref{eq:UMODEL1s} for surface values $\u(x,y,z=0)$)
and then corrupting the displacements with Gaussian white noise with standard deviation $10^{-5}$. From these noisy displacements, we reconstructed $f(\r)$.
...
\caption{\textbf{Comparing different regularizers.} Reconstruction of the test
  ...
  $\tilde{\Phi}_{\rm {L}_{2}}$. Regularization parameters chosen as in Fig.~\ref{LAMBDA}.}
\label{COMPARE}
\end{figure}
%
Figure~\ref{COMPARE} compares the reconstruction achieved from using
the different forms of $\Phi_{\rm reg}$. In all of these reconstructions, we have imposed that the surface
...
trade-off curves shown in Supplemental Fig.~\ref{LAMBDA}, and taking the value
for $\lambda$ that yields a point farthest away from the line segment
joining the ends of the plot. The chosen value
of $\lambda$ corresponds to the given balance between regularity and
data fidelity. The solution corresponding to
each particular value of $\lambda$ is shown in Fig.~\ref{COMPARE}. 
Each row in
Fig.~\ref{COMPARE} corresponds to the use of a different
regularization penalty. 

 We see in Fig.~\ref{COMPARE} that 
all forms of regularization yield reconstruction of the four pads, albeit at
different levels of scarcity in reconstruction of the surrounding regions. The
reconstruction using the
isotropic $L^1$ penalty is seen to be more sparse than that of the
isotropic $L^2$ penalty, whereas the other penalties all yielded comparable
reconstructions. In the remainder of the paper we concentrate 
on using $\tilde{\Phi}_{L^{1}}$.
...
In such inverse problems, computational complexity is a technical issue
either due to memory constraints or computational time. A tactic
...
displacement data. We coarsened the data by taking only every $n\in\{1,2,4\}$
lattice points in each dimension, noting that doing so reduces the rank
of the problem by a factor of $n^2$.
 The general features of the stress patterns are
...
are only measured along a single axis, may be used for reconstruction. Reconstructions 
of both components of the four-pad surface stress shown under measures along only $\hat{x}$
or only $\hat{y},$ using the $\tilde{\Phi}_{L^1}$ norm.}
\label{XYONLY}
\end{figure}

In some applications, displacements are only measurable along a
single axis. Reconstruction of the surface forces using this type of data
is a problem of missing data or information loss.
We tested reconstruction under these circumstances by assuming that 
...
$u_{x}$ or $u_{y}$ data. In both cases, the reconstruction of the force
in the direction perpendicular to the measured displacements is
most affected by information loss. This effect is seen in the presence of
fictitious boundary forces in both cases as the cell boundary is visible
in these reconstructions. Yet, the pads are clearly visible in both sets of
reconstructions, along both axes.
...
and no-torque constraints. Fig.~\ref{CONSTRAINTS} explores the influence
of these constraints by providing 
the differences in the reconstructions between the physically
constrained problem and unconstrained problems. In the first row, 
the force constraint is not enforced. In the second row, the torque constraint
is not enforced. In the last row, neither force nor torque constraints are enforced.
In all cases, we see that the constraints are active; removing the constraints
affect the quantitative results of the reconstruction.  In all cases,
constraints are not satisfied automatically. For example, without enforcing
the force constraint, net forces on the magnitude of $10^{-6}$ exist.
...
\caption{\textbf{Footprint misspecification.} Reconstructions performed under footprint drawn in an incorrect orientation. }
\label{FOOTPRINT}
\end{figure}

The other type of constraint that we have imposed on this problem is 
the compactness constraint that is given by enforcing zero force outside
a given cell boundary. We see from Fig.~\ref{COMPARE} that the reconstruction
of the four-pad test pattern is not likely to be sensitive to expansions of the cell boundary.
However, the effect of failing to enclose all of the pads within a given cell boundary
is unclear. In Fig.~\ref{FOOTPRINT}, we performed reconstructions where
the footprint was drawn in an incorrect orientation along the $\hat{y}$ axis,
thereby failing to localize two of the pads into the cell. In these reconstructions,
the two remaining pads are successfully recovered whereas the two 
missing pads seem to cause what looks like traction forces at the boundary 
...
\end{figure}
%
As we have done for the simulation data in Fig.~\ref{COMPARE},
the $\lambda-$optimal results for the reconstructed stress field
$\hat{\f}$ using the experimental data and the
...
In contrast with our simulated example, these results are highly dependent
on the choice of regularization. They also illustrate the importance of
using isotropic regularization, particularly in reconstruction problems using
these type of data where only unidirectional displacements are available. 
In the reconstructions using anisotropic penalties, the forces are qualitatively
distinct along the two directions. This behavior is undesirable as it appears that
the choice of observation axis heavily influences the outcome of the reconstruction 
procedure. The isotropic $L^1$ norm, by contrast, is robust.
...
coordinate rotation. The anisotropy of $\hat{\f}$ derives solely from
...
\section{Choice of regularization penalty parameter $\lambda$}

\begin{figure}[h]
\includegraphics[width=\linewidth]{fig11}
\caption{\textbf{Choice of $\lambda$} by plotting the tradeoff between regularity and data mismatch for different values of $\lambda$. The ``optimal'' value (red circle) is chosen by to be the point farthest away from the line segment joining the two ends of the plot (green and black circles).  Reconstructions under the different values of $\lambda$ given by these circles are shown. The green circle corresponds to solution with low regularity and is hence ``under-regularized.'' The black circle
coincides with a solution of high regularity and is hence ``over-regularized.''}
\label{LAMBDA}
\end{figure}
