...
\begin{proof}
Note that $u_x$ and $u_y$ are symmetric in form. Hence, it will suffice to prove just one of these assertions.
Eq.~\ref{eq:UMODEL1x} can be written as
\begin{align}
u_x(\x) &= \frac{1+\nu}{\pi E} \int \dd \x \Bigg\{ \left[ \frac{\nu(x - x')^2}{|\x-\x'|^2} + 1-\nu \right]\frac{\sigma_x(\x')}{|\x-\x'|}  \nonumber\\
&\quad+\nu\frac{(x-x')(y-y')}{|\x-\x'|^2}  \frac{\sigma_y(\x')}{|\x-\x'|}  \Bigg\} \nonumber\\
&= \frac{1}{\pi E |\x|} \Bigg( \int \dd\x 
\end{align}

 Consider the multipole expansion for $u_x$ at a fixed location $\x\not\in\Omega$, 
\begin{align}
u_x(\x) =\sum_{n=0}^\infty \frac{U_x(\theta)}{|\x|^n}, \label{eq:multipole}
\end{align}
where without loss of generality, the coordinate system is centered at some point $\mathbf{0}\in\Omega$.
